...
class SensorNode(Node):   # Create a new class called SensorNode that inherits variables & functions from Node
    ...
    def __init__(self):
        ...
        #self.publisher_ = self.create_publisher(Int16, 'param_topic', 10)  # Create a publisher for String type messages on the topic 'my_topic'
        #self.declare_parameter('my_parameter', 42)                        # Instantiate parameter, set default value to interger
        self.declare_parameter('sensor_position', 'left')                        # Instantiate parameter, set default value to interger
        self.declare_parameter('publish_rate', 2.0)                        # Instantiate parameter, set default value to interger
        GPIO.setmode(GPIO.BCM)  # Set the GPIO mode to BCM
        self.sensor_pin_mapping = {'left':17, 'right':27, 'front':22}  # Define GPIO pin mapping for sensors
        position = self.get_parameter('sensor_position').value.lower()
        rate = self.get_parameter('publish_rate').value

        if position in self.sensor_pin_mapping:
            self.sensor_pin = self.sensor_pin_mapping[position]
            GPIO.setup(self.sensor_pin, GPIO.IN)  # Set the sensor pin as input
            self.valid_position = True
            self.get_logger().info(f'Sensor position set to {position} on GPIO pin {self.sensor_pin}')
        else:
            self.get_logger().error(f'Invalid sensor position: {position}. Node will not publish data.')
            self.valid_position = False
            GPIO.cleanup()
            return
        topic_name = f'{position}_ir_sensor_data'
        self.publisher_ = self.create_publisher(Int16, topic_name, 10)
        #timer_period = 0.5                                                  # Define the timer period in seconds
        self.timer = self.create_timer(1.0, self.timer_callback)   # Create a timer that calls 'timer_callback' every 0.5 seconds

    def timer_callback(self):
        #my_param = self.get_parameter('my_parameter').get_parameter_value().integer_value
        if not self.valid_position:
            return
        msg = Int16()                                          # Create a new Int16 message
        #msg.data = my_param                                     # set msg.data to have the value of my_param
        state = GPIO.input(self.sensor_pin)  # Read the sensor state
        msg.data = 1 if state == GPIO.LOW else 0  # Set msg.data based on sensor state
        self.publisher_.publish(msg)                            # Publish the message to the topic
        self.get_logger().info(f'Publishing: [{self.get_parameter("sensor_position").value.upper()}]: DO = {msg.data}')
