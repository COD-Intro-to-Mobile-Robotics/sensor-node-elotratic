...
class SensorController(Node):  # Create a new class called SensorController that inherits variables and functions from Node

    def __init__(self):
        super().__init__('sensor_controller')          # Initialize the Node with the name 'sensor_controller'
...
def main(args=None):
    ...
    sensor_controller = SensorController()    # Create an instance of the SensorController class

    try:
        rclpy.spin(sensor_controller)          # Keep the node active and process incoming messages until interrupted
